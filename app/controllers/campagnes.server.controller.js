...
exports.list = function(req, res) { 
	Campagne.find().sort('-created').populate('user', 'displayName').exec(function(err, campagnes) {
		if (err) {
			return res.status(400).send({
				message: errorHandler.getErrorMessage(err)
			});
...
exports.campagneByID = function(req, res, next, id) { 
	Campagne.findById(id).populate('user', 'displayName').exec(function(err, campagne) {
